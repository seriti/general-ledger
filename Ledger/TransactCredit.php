    ...
    public function process()
    {
        ...
        if($mode === 'update') {
            ...
            if($direction !== 'IN' and $direction !== 'OUT') {
                $this->addError('Transaction must be IN or OUT of CREDIT account');        
            }       
            ...
            if(!$this->errors_found) {
                if($account_primary['type_id'] !== 'LIABILITY_CURRENT_CARD') {
                    $this->addError('Your CREDIT transaction account must have type = LIABILITY_CURRENT_CARD.'); 
                }
                
                //debit/credit secondary account
                //NB: Direction referes to primary CREDIT account, debit_credit to secondary account
                if($direction === 'OUT') {
                    $inc_dec = 'INCREASING'; //amount owed OR liability
                    $debit_credit = 'D'; 
                } else {
                    $inc_dec = 'DECREASING'; //amount owed OR liability
                    $debit_credit = 'C';
                }
            ...
            if(!$this->errors_found) {
                $desc = $inc_dec.' <strong>'.$account_primary['name'].'</strong> LIABILITY by amount['.$amount.'] ';
        ...
        $html .= '<form method="post" action="?mode=update" name="transact_credit">';

        $html .= '<div class="row">';
        //NB: direction refers to IN/OUT of CREDIT account
        $group_name = 'direction';
        $param = [];
        $html .= '<div class="col-sm-4"><label>'.
                  Form::radiobutton($group_name,'IN',$direction,$param).'DECREASING '.
